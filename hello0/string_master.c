...
/*** TO BE COMPLETED START ***/
  ...
  if (argc==1 || (i==0 && !strstr( argv[1] , "p" )))
    sprintf ( p ,"%c!", c[2] ); 					//maschile singolare

  else{
    if (i==0)
      sprintf	( p, "%c!", c[4] );					//maschile plurale
    else {
      if (strstr( argv[1] , "p" ))
        sprintf ( p, "%c!", c[5] );					//femminile plurale
      else
        sprintf ( p, "%c!", c[3] );					//femminile singolare
    }
  }
